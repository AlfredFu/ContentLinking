...
content="""
<a name="i26" re="T"></a>Article 26 Where the nominal shareholder transfers, pledges or otherwise disposes of the equity registered under its name and the real capital contributor claims such disposition of equity invalid on the ground that it has actual rights to the equity, the people's court may handle the case in accordance with the provisions of <a href="/law/content.php?content_type=T&origin_id=225627&provider_id=1&isEnglish=Y#i106" class="link_2_manual" >article 106</a> of the <a href="/law/content.php?content_type=T&origin_id=225627&provider_id=1&isEnglish=Y" class="link_2" re="T" cate="manual_en_href" >Real Rights Law</a> mutatis mutandis.<br />
...
def testSearch(content):
	print mhp.search(content)

def testConvertHtmlTag(content):
	return mhp.convertHTMLHyperlinkTag(content)
...
def testProcess():
	...
	article.contentType='T'
	article.originId=1232342
	article.providerId=1
	article.isEnglish='Y'
	mhp.process(article)
